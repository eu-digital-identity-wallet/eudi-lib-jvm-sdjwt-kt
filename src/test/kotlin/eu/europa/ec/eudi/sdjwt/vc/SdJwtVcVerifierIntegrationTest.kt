...
class SdJwtVcVerifierIntegrationTest {
    ...
    @Test
    fun verificationFailureDueToTypeMetadataValidationFailure() =
        runTest {
            val serialized =
                issue {
                    claim(RFC7519.ISSUER, "https://example.com/issuer")
            ...
            val expectedErrors =
                listOf(
                    ...
                    DefinitionViolation.IncorrectlyDisclosedClaim(it)
                } + DefinitionViolation.WrongClaimType(ClaimPath.claim("nationalities"))
            assertContentEquals(expectedErrors, sdJwtVcVerificationError.errors)
        }

    @Test
    fun documentIntegrityParsingSuccess() =
        runTest {
    ...
    @Test
    fun `when status reference is not present, status resolver is not invoked`() =
        runTest {
            val serialized =
                issue {
                    claim(RFC7519.ISSUER, "https://example.com/issuer")
    ...
    @Test
    fun `when status reference is not a json object, verification fails`() =
        runTest {
            val serialized =
                issue {
                    claim(RFC7519.ISSUER, "https://example.com/issuer")
    ...
    @Test
    fun `when status list reference is malformed, verification fails`() =
        runTest {
            val serialized =
                issue {
                    claim(RFC7519.ISSUER, "https://example.com/issuer")
    ...
    @Test
    fun `when status resolution fails, verification fails`() =
        runTest {
            val serialized =
                issue {
                    claim(RFC7519.ISSUER, "https://example.com/issuer")
            ...
            val checkStatus =
                CheckWithTokenStatusList { uri, index ->
    ...
    @Test
    fun `when status is non-valid, verification fails`() =
        runTest {
            val serialized =
                issue {
                    claim(RFC7519.ISSUER, "https://example.com/issuer")
    ...
    @Test
    fun `when status is valid, verification succeeds`() =
        runTest {
            val serialized =
                issue {
                    claim(RFC7519.ISSUER, "https://example.com/issuer")
