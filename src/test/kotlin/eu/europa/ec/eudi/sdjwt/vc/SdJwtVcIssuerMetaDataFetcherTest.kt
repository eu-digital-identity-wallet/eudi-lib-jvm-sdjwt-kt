...
internal class SdJwtVcIssuerMetaDataFetcherTest : GetSdJwtVcIssuerJwkSetKtorOps {
    ...
    @Test
    fun `verify issuer jwks resolution fails when issuer is mismatched`() =
        runTest {
    ...
    @Test
    fun `verify issuer jwks resolution fails when both jwks and jwks_uri are present`() =
        runTest {
            val issuer = Url("https://example.com")
    ...
    @Test
    fun `verify issuer jwks is resolved by value`() =
        runTest {
            suspend fun test(
                issuer: Url,
                metadata: Url,
                ...
                val jwks =
                    client.getSdJwtIssuerKeySet(issuer).let {
    ...
    @Test
    fun `verify issuer jwks is resolved by reference`() =
        runTest {
            suspend fun test(
                issuer: Url,
                metadata: Url,
